...
- OpenAI Chat wire codec（2026-07-27）：Google / Ollama 的 request bytes、non-stream folding、SSE chunks 與 failure semantics 先由同一組跨 adapter golden/httptest 鎖定，再收斂到 `provider/protocol/openaichat`。兩個 adapter 只保留 endpoint、default model、auth header 與 vendor error prefix；Grok、Anthropic、MiniMax、Codex、Antigravity 的 DTO 未合併。
...
│   ├── protocol/openaichat/          # Google/Ollama 共用 request/response/SSE codec
...
- Provider wire sharing（2026-07-27）：共用的條件是 observable contract 相同，不是 struct 欄位相似。Google / Ollama 由 `provider/openaichat_compat_test.go` 同時驗證 generate request bytes、stream request bytes、folded `ModelResult`/`ModelChunk`、HTTP/decode errors、malformed SSE、usage-only event、terminal chunk、context cancellation 與 scanner failure；production codec 位於 `provider/protocol/openaichat`。其他五個 adapter 保持 vendor-local DTO。
