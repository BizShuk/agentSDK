...
- OpenAI Chat wire codec（2026-07-27）：Google / Ollama 的 request bytes、non-stream folding、SSE chunks 與 failure semantics 先由同一組跨 adapter golden/httptest 鎖定，再收斂到 `provider/internal/openaichat`。兩個 adapter 只保留 endpoint、default model、auth header 與 vendor error prefix；Grok、Anthropic、MiniMax、Codex、Antigravity 的 DTO 未合併。
...
│   ├── internal/openaichat/          # Google/Ollama 共用 request/response/SSE codec；internal、不形成 public DTO
...
- Provider config pipeline（2026-07-27）：`provider.Options` 是 unresolved live input，唯一在 `Resolve(Entry.Metadata)` 查 env 並依 credential class 投影到 `ResolvedConfig.Auth`（OAuth → `Bearer`、API key → `APIKey`）；`ResolvedConfig{Model, BaseURL, Auth}` 是七個 adapter `New` 的唯一 construction input。adapter-local `config`/functional options/viper/env resolver 與 `register.go` converter 全移除。`core.Auth` 不再含 `BaseURL`：endpoint 是 construction config，不允許透過 request credential 偽裝成可輪替認證。六個遠端 adapter 以 `Metadata.CredentialRequired` 在 registry boundary fail fast；Ollama 與外部 custom entry 維持預設 credential-optional，可由 static auth 或 per-request decorator 提供認證。
- Provider wire sharing（2026-07-27）：共用的條件是 observable contract 相同，不是 struct 欄位相似。Google / Ollama 由 `provider/openaichat_compat_test.go` 同時驗證 generate request bytes、stream request bytes、folded `ModelResult`/`ModelChunk`、HTTP/decode errors、malformed SSE、usage-only event、terminal chunk、context cancellation 與 scanner failure；production codec 只存在於 `provider/internal/openaichat`。其他五個 adapter 保持 vendor-local DTO。
...
| provider registry         | `agentsdk/provider`（package `provider`，非 `registry`）：`Entry` 單獨擁有 name / metadata / static catalog / factory；`Names`/`Entries`/`Lookup`/`Catalog`/`New`/`Options.Resolve`/`ResolvedConfig`/`DEFAULT_NAME`；`env` 查詢以 `LookupEnv` 注入，CLI 傳 viper-backed、library 用 `os.Getenv` |
