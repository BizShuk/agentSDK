...
├── reasoning/                        # DecisionRule/NewDecide + 6 個純函式 FSM
...
- `reasoning/` 擁有 `DecisionRule` 與 registry-backed `NewDecide`，因為 dispatcher 是 strategy 的消費端；`DecisionRule.Kind()` 與 registry key 都直接用 `string`。六個 strategy（ThinkThenAct、PlanThenRun、RunThenReview、OneShotReasoning、LearnFromFailure、ChooseAgent）都是 phase FSM。`core` 只保留 `Decide` function type，所以 `runtime` 不必反向依賴具體 reasoning package；working memory 是 rule 與 runtime/middleware 的通訊介面。
...
| 狀態與 ports              | `agentsdk/core`：`State`/`Budget`、`Event`/`Observation`、`Instruction`/`Decide`、`Provider`/`Tool` 與 persistence/presentation ports；檔案依 domain 分組，package/API 不拆                                                                                                                                             |
| 推理策略                  | `agentsdk/reasoning`：`DecisionRule`/`NewDecide` dispatcher + 6 個 `New*` rule constructor                                                                                                                                                                                                                         |
