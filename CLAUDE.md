...
- Provider wire sharing（2026-07-27）：共用的條件是 observable contract 相同，不是 struct 欄位相似。`provider/protocol/sse` 只共享通用 framing；Google / Ollama 再由 `provider/openaichat_compat_test.go` 同時驗證 generate request bytes、stream request bytes、folded `ModelResult`/`ModelChunk`、HTTP/decode errors、malformed JSON event、usage-only event、terminal chunk、context cancellation、transport read 與 partial-frame failure。OpenAI Chat codec 位於 `provider/protocol/openaichat`；其他五個 adapter 保持 vendor-local DTO。
