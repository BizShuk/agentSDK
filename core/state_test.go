...
func TestStateClone(t *testing.T) {
	...
	assert.Equal(t, "v", clone.Scratch["k"])

	// mutate clone, original must remain
	clone.Messages[0].Chunks[0].Text = "bye"
	clone.Scratch["k"] = "modified"
	assert.Equal(t, "hi", s.Messages[0].Chunks[0].Text)
	assert.Equal(t, "v", s.Scratch["k"])
}
...
func TestStateJSONRoundTrip(t *testing.T) {
	in := core.State{
		...
		Messages: []core.Message{
			{Role: core.ROLE_USER, Ts: time.Unix(1700000000, 0).UTC(),
				Chunks: []core.Chunk{{Kind: core.CHUNK_KIND_TEXT, Text: "hello"}}},
		},
	...
	assert.Equal(t, in.ThinkingKind, out.ThinkingKind)
	require.Len(t, out.Messages, 1)
	assert.Equal(t, "hello", out.Messages[0].Chunks[0].Text)
