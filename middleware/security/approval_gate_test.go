...
func (r *recordingPolicy) Decide(_ struct{}, _ core.AutonomyLevel, _ core.CallToolInstruction, _ core.ToolSpec) core.ApprovalAction {
	r.called = true
	return r.verdict
}
...
func TestApprovalGateAllowPassesThrough(t *testing.T) {
	...
	assert.Equal(t, core.INSTRUCTION_CALL_TOOL, seen.Kind)
...
func TestApprovalGateAskRewritesToRequestApproval(t *testing.T) {
	...
	assert.Equal(t, core.INSTRUCTION_REQUEST_APPROVAL, seen.Kind)
...
func TestApprovalGateDenyEmitsNotify(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, core.INSTRUCTION_NOTIFY, seen.Kind)
