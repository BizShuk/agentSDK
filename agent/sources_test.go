...
func TestPersonaSourceUsesTheStableOrder(t *testing.T) {
	got := sectionsOf(t, agent.PersonaSource("you are terse"), prompt.Req{})
	require.Len(t, got, 1)
...
func TestPersonaSourceEmptyContributesNothing(t *testing.T) {
	assert.Empty(t, sectionsOf(t, agent.PersonaSource(""), prompt.Req{}))
}
...
func TestEnvSourceIsLastAmongSystemSections(t *testing.T) {
	got := sectionsOf(t, agent.EnvSource(), prompt.Req{Cwd: t.TempDir()})
	require.Len(t, got, 1)
...
func TestReminderSourceOnlySpeaksNearTheBudget(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := sectionsOf(t, agent.ReminderSource(), prompt.Req{
