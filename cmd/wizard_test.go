...
// runWizard executes the command with the given args and stdin, returning
// stdout. Prompts go to stderr, so stdout stays parseable even when the
// wizard is chatty.
...
func TestWizardEveryTierProducesAValidConfig(t *testing.T) {
	// Pressing Enter through every stage must yield something the builder
	// accepts. If a default were invalid, the wizard's happy path would
...
func TestWizardOneshotStaysNameless(t *testing.T) {
	// The name question only fires when something persists, so the
	// cheapest tier keeps its "no required fields" promise.
...
func TestWizardListsChoicesFromTheSharedMetadata(t *testing.T) {
	cases := []struct {
		...
		{"model.provider", nil}, // registry-driven; checked for non-empty below
...
func TestWizardInteractiveSelectionByNumberAndByName(t *testing.T) {
	// stage 0 tier=2 (basic), stage 1 provider="anthropic" typed out,
	// then EOF — which the wizard treats as "take the rest of the
	// defaults" rather than failing halfway through.
...
func TestWizardAlwaysRegistersTheSelectedStyle(t *testing.T) {
	// Choosing a style but not listing it under "also register" would
	// make the engine dispatch to an unregistered rule. The wizard adds
	// it back rather than letting validation reject the answers.
	out, _, err := runWizard(t, "2\n\n\n\n6\n1\n", "-o", "-")
	require.NoError(t, err)
