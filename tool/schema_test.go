...
func TestSchemaJSONRoundTrip(t *testing.T) {
	raw, err := tool.SchemaJSON[schemaArgs]()
	require.NoError(t, err)
...
func TestSchemaForToolProducesCompleteToolSchema(t *testing.T) {
	ts, err := tool.SchemaForTool[schemaArgs]("read_file", "read a file", core.RISK_LEVEL_HIGH)
	require.NoError(t, err)
...
func TestValidateArgsAcceptsCompletePayload(t *testing.T) {
	ok, err := tool.ValidateArgs[schemaArgs]("read_file", json.RawMessage(`{"path":"/tmp/x"}`))
	assert.True(t, ok)
	assert.Nil(t, err)
}

func TestValidateArgsRejectsMissingRequired(t *testing.T) {
	ok, err := tool.ValidateArgs[schemaArgs]("read_file", json.RawMessage(`{"mode":"fast"}`))
	assert.False(t, ok)
	require.NotNil(t, err)
	var se *tool.SchemaError
	require.ErrorAs(t, err, &se)
	assert.Contains(t, err.Error(), "missing required field: path")
}

func TestValidateArgsRejectsInvalidJSON(t *testing.T) {
	ok, err := tool.ValidateArgs[schemaArgs]("read_file", json.RawMessage(`not json`))
	assert.False(t, ok)
	require.NotNil(t, err)
}

func TestValidateArgsRejectsEmptyObjectForRequiredOnly(t *testing.T) {
	ok, err := tool.ValidateArgs[onlyRequired]("rename", json.RawMessage(`{}`))
	assert.False(t, ok)
	assert.Contains(t, err.Error(), "missing required field: name")
}

func TestValidateArgsAcceptsEmptyObjectForAllOptional(t *testing.T) {
	ok, err := tool.ValidateArgs[onlyOptional]("labeler", json.RawMessage(`{}`))
	assert.True(t, ok)
	assert.Nil(t, err)
}
...
func TestRegisterFuncSchemaAutoReflected(t *testing.T) {
	r := tool.NewRegistry()
	tool.RegisterFunc(r, "read_file", "read a file", core.RISK_LEVEL_LOW,
