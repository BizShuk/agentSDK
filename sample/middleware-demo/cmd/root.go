...
// Version 是 middleware-demo 的版本字串。
const Version = "0.1.0"

// NewRoot 組出 middleware-demo 的指令樹。
//
//	middleware-demo           # 跑全部五個 demo
//	middleware-demo list      # 列出 demo id
//	middleware-demo run <id>  # 只跑一個
...
middleware-demo 把 M2 middleware 鏈的每一層各跑一次:retry、budget、
...
base dispatcher 收尾,因此離線、決定性、不真的 sleep。`),
	...
	root.SetVersionTemplate("middleware-demo {{.Version}}\n")
...
func newRunCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return fmt.Errorf("unknown demo %q — run `middleware-demo list` to see the ids", id)
		},
	}
}
