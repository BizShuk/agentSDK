// Package cmd hosts the file-agent CLI.
package cmd
...
func NewRoot() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "file-agent",
		Short:   "Operate on files with the six built-in tools — an agentsdk demo",
		Version: Version,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			slog.Info("file-agent starting", "target", target, "runID", cfg.RunID)
			...
			// --- tool registry ---
			// Built-in tools only (Read/Write/Edit/Bash/Glob/Grep) — no
			// custom tool, demonstrating that the six defaults suffice to
			// drive a file-operating agent. Sandbox policy is the default
			// denylist + /tmp allowlist; high-risk tools (Write/Edit/Bash)
			// gate on HITL at L2 via the SecureMiddleware approval gate.
			...
			state := core.State{
				...
				Messages: []core.Message{{
					Role: core.ROLE_USER,
					Parts: []core.Part{
						{Kind: core.PART_KIND_PLAIN_TEXT, Text: fmt.Sprintf("%s\nTarget path: %s", task, target)},
					},
					Ts: time.Now().UTC(),
				}},
			...
			slog.Info("file-agent done", "runID", cfg.RunID, "turns", final.Turn, "status", final.Status)
			_ = final
			return nil
		},
	}

	cmd.SetVersionTemplate("file-agent {{.Version}}\n")
	cmd.Flags().StringVar(&target, "target", ".", "Path (file or directory) the agent should operate on.")
	cmd.Flags().StringVar(&task, "task", "Explore the files in the target path and summarize what you find.",
	...
	cmd.Flags().IntVar(&maxTurns, "max-turns", 15, "Maximum steps the agent can take before being killed.")
