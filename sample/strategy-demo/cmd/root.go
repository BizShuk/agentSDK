...
// Version 是 strategy-demo 的版本字串。
const Version = "0.1.0"

// NewRoot 組出 strategy-demo 的指令樹。
//
//	strategy-demo            # 追蹤全部六條策略
//	strategy-demo list       # 列出策略 id 與標題
//	strategy-demo run <id>   # 只追蹤一條
...
strategy-demo 把 agentsdk 的六條 planning 策略各驅動一次,印出 phase 轉移
...
不需要 API key。`),
	...
	root.SetVersionTemplate("strategy-demo {{.Version}}\n")
...
func newRunCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return fmt.Errorf("unknown strategy %q — run `strategy-demo list` to see the ids", id)
		},
	}
}