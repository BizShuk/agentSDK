...
// Version 是 memory-demo 的版本字串。
const Version = "0.1.0"

// NewRoot 組出 memory-demo 的指令樹。
//
//	memory-demo               # 跑全部三個 demo
//	memory-demo list          # 列出 demo id
//	memory-demo run <id>      # 只跑一個
...
memory-demo 把 agentsdk 支柱 2 (memory) 的三個子元件各跑一次:
...
的快照 + WAL 重放。全部離線、決定性,不需要 provider 或 API key。`),
	...
	root.SetVersionTemplate("memory-demo {{.Version}}\n")
...
func newRunCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return fmt.Errorf("unknown demo %q — run `memory-demo list` to see the ids", id)
		},
	}
}
