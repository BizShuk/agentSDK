...
| `vocabulary owner`  | name-to-implementation owner | 純名稱到實作的 mapping 留在詞彙所屬 package：tools 用 `builtin.Register`、reasoning 用 `reasoning.NewRule`、autonomy 用 `core.ParseAutonomyLevel`；`agent` 只組裝                                     | `tool/builtin/builtin.go`、`reasoning/decision.go`、`core/autonomy.go`            |
| `build pipeline`    | 7-stage build pipeline       | `agent` 的組裝順序：provider → tools → reasoning → prompt → safety → memory → assemble                                                                                                           | `agent/build.go::Bootstrap`                                                       |
| `wizard stage`      | 9-stage configuration flow   | wizard 的設定流程：tier → model → reasoning → tools → safety → subagents → memory → limits → review；其階段數不等同 7-stage build pipeline                                                       | `cmd/agent/wizard/wizard.go::run`                                                 |
...
| `provider registry` | provider registry            | provider 名稱到 adapter factory 的唯一映射；`Names`、`Entries`、`Lookup` 與 `New` 供 CLI 與 agent 共用                                                                                           | `provider/registry.go`                                                            |
| `Capability`        | operation capability         | provider operation 的 canonical vocabulary：`chat`、`catalog`、`image`、`video`、`music`、`transcribe`、`speech`、`live`、`translate`；不表示輸入或輸出 data form                              | `provider/capability.go`                                                          |
| `provider support`  | provider-level support       | `Entry` 是否具備該 capability 的 factory；用來判斷 provider 能否建構對應 typed client                                                                                                           | `provider/capability.go::Entry.Supports`                                          |
| `model support`     | model-level support          | `ModelSpec.Capabilities` 是否宣告該 model 能執行 operation；與 provider support 取交集後才是可選組合                                                                                            | `provider/model.go::ModelSpec.Supports`                                           |
| `Modality`          | data modality                | `text`、`image`、`audio`、`video` 四種 data form；`InputModalities` 表示 model 能讀取，`OutputModalities` 表示 model 能產生，不等同 capability                                                   | `provider/model.go`                                                               |
| `provider catalog`  | bundled model catalog        | `Entry.Catalog` 提供的靜態 `provider.ModelSpec` 清單；wizard 只列 `chat` models，其他 consumer 可依 capability 與 directional modalities 篩選                                                    | `provider/registry.go::Catalog`、`cmd/agent/wizard/notes.go`                      |
| `pricing snapshot`  | checked-in price catalog     | 由 OpenRouter public model manifest 正規化的 per-unit USD prices；只用於估價，不取代 provider 回傳的 authoritative billed cost                                                                    | `provider/pricing/snapshot.json`、`provider/pricing/refresh.go`                   |
| `applicability`     | benchmark applicability      | model capability 已成立後，現有 benchmark case 是否具備該 model 要求的具體輸入；缺少 subject/reference asset 時只排除 benchmark case，不否定 provider/model capability                            | `benchmark/applicability.go`                                                      |
| `MusicGenerator`    | music generation capability  | provider layer 的 non-streaming music optional capability；由 `provider.NewMusic` 建構，與 agent runtime 及 generic audio contract 分離                                                       | `provider/music.go`、`provider/registry.go::NewMusic`                             |
| `ResolvedConfig`    | resolved construction input  | registry 解析完成後交給 adapter `New` 的唯一 construction shape：`Model`、`BaseURL`、`Auth`；不含 env lookup 或 decorator                                                                        | `provider/registry_options.go::ResolvedConfig`                                    |
| `Resolve`           | environment resolution       | `provider.Options.Resolve` 依 entry metadata 將 OAuth env 投影到 `Auth.Bearer`、API key env 投影到 `Auth.APIKey`，endpoint 放入 `ResolvedConfig.BaseURL`；不負責設定檔載入或 token refresh          | `provider/registry_options.go::Options.Resolve`                                   |
| `LookupEnv`         | environment lookup injection | `provider.Options.LookupEnv func(string) string`：CLI 可傳 viper-backed lookup，library 預設用 `os.Getenv`                                                                                       | `provider/registry_options.go::Options`                                           |
| `Once`              | one-shot facade              | `agent.Once(ctx, cfg, prompt)` 強制使用 `oneshot` tier 與 `OneShotReasoning`，仍經 `runtime.Engine` 執行；tools 與 persistence 關閉，但可注入 provider、sink 或 notifier，且不需建立 `Host`      | `agent/once.go::Once`、`agent/once.go::oneshot`                                   |
...
| `EventSink`       | (—)              | 呈現流 port；frontend 以 `agent.WithSink` 或直接設定 `Engine.Sink` 注入，`agent/wire` 提供可選的 JSONL adapter                       | `core/stream.go`、`agent/options.go`、`agent/wire/sink.go`                |
| `TokenUsage`      | token usage       | 單次 response 或整個 run 的 canonical token counters：input、output、cache read、web search 與 total                              | `core/model.go`、`core/state.go`                                           |
| `Cost`            | USD cost metadata | 固定小數 USD amount、status、pricing timestamp 與 source；可代表 provider exact cost、snapshot estimate、free 或 unpriced          | `core/cost.go`                                                             |
| `CostStatus`      | cost confidence   | `exact`、`estimated`、`free`、`unpriced` 四種成本語意；`unpriced` 不等於零成本                                                     | `core/cost.go`                                                             |
